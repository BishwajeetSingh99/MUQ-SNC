...
def run_sim(index,case,input_,caseID,path,total):
	try:
		...
		#print(os.listdir())
		try:
			result = subprocess.call(['python3.12','cantera_1.py','&>solve_1.out'])#, check=True, text=True, capture_output=True)
			...
			if "tau.out" in os.listdir(path):
				...
				try:
					...
					try:
						...
						result = subprocess.run(['python3.12'] +argv, capture_output=True, text=True)
						print(result)
						print(result.stdout)
						if result.stderr:
						#	print("Errors:")
