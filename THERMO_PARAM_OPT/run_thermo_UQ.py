...
import Uncertainty as uncertaintye
...
'''
# checking the model prediction for GA iterations 

ga_input_file = "GA_INPUTS_SHARED.csv"  # <- change this to your actual file name

try:
    ga_inputs = np.loadtxt(ga_input_file, delimiter=",")
except Exception as e:
    print(f"Error loading GA input file: {e}")
    raise

# Ensure inputs are 2D (in case there's only one row)
if ga_inputs.ndim == 1:
    ga_inputs = ga_inputs.reshape(1, -1)

# Output folder for predictions
output_folder = "GA_PREDICTIONS"
os.makedirs(output_folder, exist_ok=True)

# Loop over all trained response surfaces
for case_index, response_model in ResponseSurfaces.items():
    
    try:
        predictions = [response_model.evaluate_prs(x) for x in ga_inputs]
    except Exception as e:
        print(f"Prediction failed for case {case_index}: {e}")
        continue

    # Save predictions to file
    output_file = os.path.join(output_folder, f"ga_predictions_case-{case_index}.txt")
    np.savetxt(output_file, predictions, fmt="%.6f")
    
    print(f"Saved predictions for case {case_index} → {output_file}")
raise AssertionError("stop")
'''
